...
export const useOpenAIAssistant = () => {
  ...
  const pollRunStatus = async (
    ...
    while (attempts < maxAttempts) {
      ...
      attempts++;
      console.log(
        `Polling attempt ${attempts}/${maxAttempts} for run ${runId}, status: ${status.status}`
      );
    }
  ...
  const runAssistantWithInstructions = async (
    threadId: string,
    ...
    if (questionNumber === 1) {
      ...
      instructions =
        "Generate a final interpretation with: (1) A detailed explanation, (2) A relevant Quranic verse in Arabic with translation, and (3) A spiritual conclusion.";
    }

    console.log(
      `Running assistant with instructions for question ${questionNumber}: "${instructions}"`
    );
    ...
    console.log(
      "Run started with ID:",
      run.id,
      "with instructions for question",
      questionNumber
    );
    return run;
  };
  ...
  const runAssistantAndGetResponse = async (
    threadId: string,
    ...
    try {
      setIsLoading(true);
      console.log(
        `Running assistant for question ${questionNumber} on thread ${threadId}`
      );
  ...
  // Save final interpretation to Supabase
